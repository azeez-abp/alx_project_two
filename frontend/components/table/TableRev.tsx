...
// Define types for columns and revenues
interface Irevenue {
...
const columns: Column[] = [
  ...
  { uid: "source", name: "Source", sortable: true },
...
const INITIAL_VISIBLE_COLUMNS = ["amount", "source", "actions"];
...
export default function TableRevenue() {
  ...
  const filteredItems = useMemo(() => {
    let filteredrevenues = revenue !== null ? revenue : [];

    if (hasSearchFilter) {
      filteredrevenues = filteredrevenues.filter((p) =>
        p.category.toLowerCase().includes(filterValue.toLowerCase())
      );
    }

    return filteredrevenues;
  }, [revenue, filterValue, statusFilter]);
  ...
  const sortedItems = useMemo(() => {
    return [...items].sort((a, b) => {
      const first = a[sortDescriptor.column as keyof Irevenue];
      const second = b[sortDescriptor.column as keyof Irevenue];
  ...
  const classNames = useMemo(
    () => ({
      wrapper: ["max-h-[382px]", "max-w-4/5", "twraper", "revenue-table"],
  ...
  const renderCell = useCallback((revenue: Irevenue, columnKey: string) => {
    const cellValue = revenue[columnKey as keyof Irevenue];
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        <div className="flex justify-between items-center">
          <span className="text-default-400 text-small">Total {revenue ? revenue.length : 0} revenues</span>
  ...
  }, [filterValue, visibleColumns, onRowsPerPageChange, revenue ? revenue.length : 0, onSearchChange, hasSearchFilter]);

  const bottomContent = useMemo(() => {
    return (
      <div className="py-2 px-2 flex justify-between items-center">
        <span className="w-[30%] text-small text-default-400">
          {selectedKeys.size === (revenue ? revenue.length : 0)
            ? "All items selected"
  ...
  }, [selectedKeys, items.length, page, pages, hasSearchFilter, revenue]);
    ...
    revenue && (
      <div className="flex w-[90%]">
        ...
        >
          ...
          <TableBody emptyContent="No revenue found" items={sortedItems}>
            {(item) => (
