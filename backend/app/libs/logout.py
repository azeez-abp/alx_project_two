...
from sqlalchemy import select, delete    # type: ignore                                                                                                                                                                                                                                                                              # type: ignore
...
def logout(request):
    ...
    try:
        ...
        session.execute(
                  delete(Session).where(Session.session_id == user_id)
        )
        session.commit()
        # Clear the cookie
        CookieHandler.set_cookie(request, getenv("SESSION_COOKIE_NAME"), '', -7*24*60*60)
