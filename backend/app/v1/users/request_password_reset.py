    ...
    def post(self):
        ...
        try:
            ...
            if has_session is None:
                return responseObject(False, True, f'User with email {req_data.get("email")} not found'), 404

            send_mail = EmailService("adioadeyoriazeez@gmail.com",
                                    "trrqdlvqadasyxqe")
            mail = send_mail.send_html_email("OTP for Password reset", f'<h1>{otp} </h1>',
                                    req_data.get("email"))
            if mail is True:
...
class PasswordRest(Resource):
    def post(self):
        ...
        try:
            ...
            if has_session is None:
                return responseObject(False, True, f'User with opt email {req_data.get("otp")} not found'), 404
